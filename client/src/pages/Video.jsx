...
const Video = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const videoRes = await axios.get("https://www.googleapis.com/youtube/v3/",
          {params: {
        ...
        const channelRes = await axios.get("https://www.googleapis.com/youtube/v3/",
          {
            params: {
              part: "snippet,contentDetails,statistics",
              type: "channel",
        ...
        setChannel(channelRes?.data.items);
  ...
  const handleSub = async () => {
    ...
    if (!currentUser) {
      // You can customize the alert or redirect to the sign-in page
    ...
    try {
      const response = currentUser.subscribedUsers.includes(channel?._id)
        ? await axios.put(`/users/unsub/${channel?._id}`)
        : await axios.put(`/users/sub/${channel?._id}`);
